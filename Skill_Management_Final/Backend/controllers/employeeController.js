...
// Assuming you have your PostgreSQL connection pool (pool) set up
// Assuming you have middleware for authentication (optional, but recommended)

exports.getCompletedTrainingForEmployee = async (req, res) => {
  const employeeId = req.params.id; // Assuming the employee ID is passed as a route parameter (e.g., /employees/:id/completed-training)
  ...
  try {
    ...
    const trainingData = result.rows[0]; // Expecting a single completed training record

    if (trainingData) {
      res.json(trainingData);
    } else {
      res.status(404).json({ message: `No completed training found for employee ${employeeId}` });
    }
  } catch (error) {
...
// const isAuthenticated = require('../middleware/isAuthenticated'); // Optional authentication middleware

// // Apply authentication middleware if needed
// //router.get('/employees/:id/completed-training', isAuthenticated, employeeController.getCompletedTrainingForEmployee);

// module.exports = router;

// Get Employee Profile by emp_id
exports.getEmployeeProfilec = async (req, res) => {
  const { emp_id } = req.params;
  try {
    const result = await pool.query(`
      SELECT
      ...
      LEFT JOIN
        ...
        AND e.skill_level = ta.skill_level
      WHERE
        e.is_deleted = false AND e.emp_id = $1
      ORDER BY
        e.emp_id;
    `,[emp_id]);
    res.json(result.rows);  // Send back the query result
...
exports.updateEmployeeClient = async (req, res) => {
  const { emp_id } = req.params;
