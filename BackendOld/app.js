...
//****************************************************** */
app.post('/employee-dashboard', async (req, res) => {
  const { email, password } = req.body;
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT 
        ...
        s.name AS "PrimarySkills", 
        sc.name AS "PrimarySkillCategoryName", 
        sl.level_name AS "SkillLevel", 
        t.training_name AS "AssignedTraining", 
        et.completionstatus AS "TrainingStatus"
      FROM 
        Employee e
      JOIN 
        Department d ON e.department_id = d.dept_id
      JOIN 
        Skill s ON e.primary_skill_id = s.skill_id
      JOIN 
        SkillCategory sc ON s.category_id = sc.skill_category_id
      JOIN 
        Employee_Skill es ON e.emp_id = es.employee_id AND es.skill_id = e.primary_skill_id
      JOIN 
        SkillLevel sl ON es.skill_level_id = sl.skill_level_id
      JOIN 
        EmployeeTrainings et ON e.emp_id = et.EmployeeId
      JOIN 
        Training t ON et.TrainingId = t.training_id
      WHERE 
        e.emp_id = $1 AND e.isDeleted = FALSE;
    `, [email]);  // Use the logged-in employee's email (or emp_id if available)

    if (result.rows.length > 0) {
      res.json(result.rows[0]);
    } else {
      res.status(404).json({ error: 'Employee not found or invalid credentials' });
    }
  } catch (error) {
    console.error(error);
